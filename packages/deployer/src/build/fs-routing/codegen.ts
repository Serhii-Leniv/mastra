...
export async function generateFsAgentsModule(
  ...
  const wfCodegen = workflows.length > 0 ? generateFsWorkflowsCodegen(workflows, standalone) : undefined;
 ...
 */
export function generateFsWorkflowsCodegen(workflows: DiscoveredFsWorkflow[], standalone?: boolean): {
  importLines: string[];
